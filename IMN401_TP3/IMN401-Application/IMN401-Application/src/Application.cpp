...
*********************/
	...
	glm::vec2 diff = v - o;
	glm::vec3 direction = this->m_scene->camera()->frame()->convertDirTo(glm::vec3(diff, 0), f);

	glm::vec3 zero = this->m_scene->camera()->frame()->convertPtFrom(glm::vec3(0, 0, 0), f);
	glm::vec3 length1 = this->m_scene->camera()->frame()->convertDirFrom(glm::vec3(0, 0, 1), f);

	f->translate(direction / glm::length(length1) * glm::length(zero));*/

	/*float distance = direction.length();
	glm::vec3 translation_vector = direction * distance;

	Node* noeud = this->m_scene->getManipulatedNode();
	translation_vector = noeud->frame()->convertDirTo(direction, noeud->frame());
	noeud->frame()->translate(translation_vector);*/
...
*********************/
	...
	//glm::vec3 perpendiculaire = v1 * v2;
	glm::vec3 perpendiculaire = glm::cross(v1, v2);

	float angle = glm::length(perpendiculaire);

	perpendiculaire /= angle;
	...
	noeud->frame()->rotate(perpendiculaire, angle);
...
void Application::animate(const int elapsedTime) 	
{
